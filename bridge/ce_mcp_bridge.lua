...
-- ============================================================================
-- NEW TOOLS (v12) - Cheat Table, Lua Output, UTF-16, Quick Attach Check
-- ============================================================================

-- Capture buffer for Lua print() output
local luaOutputBuffer = {}
...
print = function(...)
    ...
    luaOutputBuffer[#luaOutputBuffer + 1] = line
    -- Still print to CE console
    originalPrint(...)
end

local function cmd_get_lua_output(params)
    local output = table.concat(luaOutputBuffer, "\n")
    if params.clear then
        luaOutputBuffer = {}
    end
    return { success = true, output = output, line_count = #luaOutputBuffer }
end

local function cmd_execute_ct_entry(params)
    local entry = params.entry
    if not entry then return { success = false, error = "No entry provided" } end
    local active = params.active
    if active == nil then return { success = false, error = "No active state provided" } end

    local al = getAddressList()
    local count = al.Count
    local memrec = nil

    -- Try by description (name) first
    for i = 0, count - 1 do
        local mr = al.getMemoryRecord(i)
        if mr.Description == entry then
            memrec = mr
            break
        end
    end

    -- Try by ID if name didn't match
    if not memrec then
    ...
    if not memrec then
        return { success = false, error = "Entry not found: " .. tostring(entry) }
    end

    memrec.Active = active
    return { success = true, entry = memrec.Description, id = memrec.ID, active = memrec.Active }
end

local function cmd_get_ct_entries(params)
    local al = getAddressList()
    local count = al.Count
    local entries = {}
    for i = 0, count - 1 do
        ...
        entries[#entries + 1] = {
            ...
            active = mr.Active,
            address = mr.Address or "",
            value = mr.Value or "",
            type = mr.Type or 0,
        }
    end
    return { success = true, count = count, entries = entries }
end

local function cmd_freeze_address(params)
    local address = params.address
    local value = params.value
    local vtype = params.type or "dword"
    ...
    if not freeze then
        -- Find and remove existing frozen entry for this address
        for i = al.Count - 1, 0, -1 do
            local mr = al.getMemoryRecord(i)
            if mr.Address == address and mr.Description == "[MCP Freeze] " .. address then
                mr.Active = false
                mr.destroy()
                return { success = true, action = "unfrozen", address = address }
            end
        end
        return { success = false, error = "No frozen entry found for " .. address }
    end

    -- Create a new memory record with freeze
    local mr = al.createMemoryRecord()
    mr.Description = "[MCP Freeze] " .. address
    mr.Address = address
    mr.Value = tostring(value)

    -- Map type string to CE type enum
    local typeMap = { byte = 0, word = 1, dword = 2, qword = 6, float = 3, double = 4 }
    mr.Type = typeMap[vtype] or 2

    mr.Active = true
    mr.Frozen = true

    return { success = true, action = "frozen", address = address, value = tostring(value), type = vtype }
end

local function cmd_read_utf16_string(params)
    local address = params.address
    if not address then return { success = false, error = "No address provided" } end
    local maxLen = params.max_length or 256

    local addr = getAddress(address)
    if not addr or addr == 0 then
        return { success = false, error = "Invalid address: " .. tostring(address) }
    end

    local utf8_parts = {}
    local char_count = 0
    for i = 0, maxLen - 1 do
        local word = readSmallInteger(addr + i * 2)
        if not word or word == 0 then break end
        -- readSmallInteger returns signed; convert to unsigned for codepoints > 0x7FFF
        if word < 0 then word = word + 65536 end
        char_count = char_count + 1
        -- Encode UTF-16 codepoint as UTF-8
        if word < 0x80 then
            utf8_parts[#utf8_parts + 1] = string.char(word)
        elseif word < 0x800 then
            utf8_parts[#utf8_parts + 1] = string.char(
                0xC0 + math.floor(word / 64),
                0x80 + (word % 64)
            )
        else
            utf8_parts[#utf8_parts + 1] = string.char(
                0xE0 + math.floor(word / 4096),
                0x80 + (math.floor(word / 64) % 64),
                0x80 + (word % 64)
            )
        end
    end

    return { success = true, address = string.format("%X", addr), text = table.concat(utf8_parts), length = char_count }
end

local function cmd_get_attached_process_quick(params)
    local pid = getOpenedProcessID()
    if not pid or pid == 0 then
        return { success = true, attached = false, pid = 0, name = "" }
    end
    return { success = true, attached = true, pid = pid, name = process or "" }
end
...
local commandHandlers = {
    ...
    -- New tools (v12)
    get_lua_output = cmd_get_lua_output,
    execute_ct_entry = cmd_execute_ct_entry,
    get_ct_entries = cmd_get_ct_entries,
    freeze_address = cmd_freeze_address,
    read_utf16_string = cmd_read_utf16_string,
    get_attached_process_quick = cmd_get_attached_process_quick,
}
